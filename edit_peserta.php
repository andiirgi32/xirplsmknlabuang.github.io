  ...
  if (isset($_GET['id'])) {
    ...
    $data = mysqli_fetch_assoc($result);
      // apabila data tidak ada pada database maka akan dijalankan perintah ini
       if (!count($data)) {
          echo "<script>alert('Data tidak ditemukan pada database');window.location='absen.php';</script>";
       }
  } else {
    // apabila tidak ada data GET id pada akan di redirect ke index.php
    echo "<script>alert('Masukkan data id.');window.location='absen.php';</script>";
  }         
  ?>
...
<html>
  ...
  <body>
      <center>
        <h1>Edit Peserta <?php echo $data['nama']; ?></h1>
      <center>
      ...
      <section class="base">
        ...
        <div>
         ...
         <input type="text" name="jk" required=""value="<?php echo $data['jk']; ?>" />
        </div>
        <div>
          <label>Kelas</label>
